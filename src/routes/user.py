...
async def user_create_route(user: UserCreate, db=Depends(get_database)):
...
async def user_update_route(id: str, user: UserUpdate, db=Depends(get_database)):
    if user.username:
        existing_username = check_username_exists(user.username)
        if existing_username:
            raise HTTPException(
...
async def user_delete_route(username: str, db=Depends(get_database)):
